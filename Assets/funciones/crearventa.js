        ...
        "ajax":
        {
            ...
            error:function(e){
                ...
                try {
                    ...
                    console.error("Failed to parse error response:", e.responseText);
                }
            }
        },
        "bDestroy":true,
...
function numero_comp(){
    ...
    $.ajax({
        ...
        error: function(e) {
            console.error("Error getting document number:", e.responseText);
        }
    });
}
...
function mostrar_impuesto(){
    $.ajax({
        ...
        error: function(e) {
            console.error("Error getting tax info:", e.responseText);
        }
    });
}
...
function addDetail(id,producto,concentracion,adicional,precio,cantidad){
    ...
    var descuento=0;
    var price = parseFloat(precio) || 0;

    if (id!="") {
        var subtotal=numero_cantidad*price;
        ...
        '<td class="text-center"><input class="form-control text_fondo" type="text" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" onchange="modifySubtotals()" name="precio_venta[]" id="precio_venta[]" step="0.01" value="'+price.toFixed(2)+'"></td>'+
        ...
        Swal.fire({
            title: 'Error!',
            text: 'Error al ingresar el detalle, revise los datos del artículo',
            icon: 'error',
...
function modifySubtotals(){
    ...
    for (var i = 0; i < cant.length; i++) {
        var inpV=parseFloat(cant[i].value) || 0;
        var inpP=parseFloat(prev[i].value) || 0;
        var des=parseFloat(desc[i].value) || 0;
        var subtotal = (inpV * inpP) - des;
        
        document.getElementsByName("subtotal")[i].innerHTML = subtotal.toFixed(2);
    }
...
function calculateTotals(){
    ...
    if(sub.length){
        for (var i = 0; i < sub.length; i++) {
            total += parseFloat(sub[i].innerHTML) || 0;
        }
        igv=total*(no_aplica/100);
        total_monto=total+igv;
        ...
        $.ajax({
            ...
            error: function(e) {
                console.error("Error getting currency symbol:", e.responseText);
            }
        });
    } else {
        $("#total").html('0.00');
...
function nombre_impuesto(){
    $.ajax({
        ...
        error: function(e) {
            console.error("Error getting tax name:", e.responseText);
        }
    });
}
...
$(document).on('submit', '#form', function(e){
    ...
    if(cliente != '0' && fecha != '' && tipo_comprobante != '' && serie_comprobante != '' && num_comprobante !='' && impuesto != ''){
        ...
        $.ajax({
            ...
            error: function(e){
                console.error("Error submitting form:", e.responseText);
                $("#ajaxAnswer").html(msjError);
            }       
        });
...
function cleanCart() {
    $.ajax({
        ...
        error: function(e) {
            console.error("Error cleaning cart:", e.responseText);
        }
    });
}
...
$(document).on('submit', '#formCliente', function(e){
    ...
    if(nombre != '' && dni != '' && direccion != '' && celular != '' && correo != ''){
        ...
        $.ajax({
            ...
            error: function(e){
                $('#Modalcliente').modal('hide');
                console.error("Error adding client:", e.responseText);
    ...
    for (let i = 0; i < $('.filas').length; i++) {
        ...
        if(element) {
            for (let f = 0; f < $('tbody tr[role="row"]').length; f++) {
                ...
                if(button && button['name'] === element['value']){
...
$('body').barcodeListener().on('barcode.valid', function(e, code){
    $.ajax({
            ...
            if(resp.length === 0){ 
                ...
                if(resp[0].cantidad !== null && resp[0].cantidad > 0){
                    for (let i = 0; i < $('.filas').length; i++){
                        ...
                        if(element && element['value'] === resp[0].id)
                        {
                            Swal.fire({
                                title: 'Oops',
        ...
        error: function(e) {
            console.error("Error scanning barcode:", e.responseText);
        }
    });
});
    ...
    $.ajax({
        ...
        success: function (resp) {
            try {
                ...
                console.error("Error parsing cart data:", resp);
            }
        },
        error: function(e) {
            console.error("Error loading cart:", e.responseText);
        }
    });
}
...
function remove(element){
    $.ajax({
        ...
        error: function(e) {
            console.error("Error removing item from cart:", e.responseText);
        }
    });
}
