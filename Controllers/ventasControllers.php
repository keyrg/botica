...
class ventasControllers extends ventasModels{
    ...
    public function list_prod_controller(){
        ...
        while ($reg=$query->fetch()) {
            ...
            $stockData = $query2->fetch();
            
            $stock = 0;
            $cantidad = 0;
            
            if(!empty($stockData['lote_stock'])){
                $stock = $stockData['lote_stock'];
                $cantidad = $stock;
                
                if($stock >= 15){
                    $stockHtml = '<span class="badge badge-info">'.$stock.'</span>';
                } else {
                    $stockHtml = '<span class="badge badge-warning">'.$stock.'</span>';
                }
            } else {
                $stockHtml = '<span class="badge badge-danger">0</span>';
                continue; // Saltar productos sin stock
            }
            
            // Asegurar que el precio tenga un valor
            $precio = isset($reg['prod_precioV']) ? $reg['prod_precioV'] : 0;
            
            $data[] = array(
                "0" => '<img src="'.$reg['prod_imagen'].'" width="40px">',
                "1" => $reg['prod_nombre'].' '.$reg['prod_concentracion'].' '.$reg['prod_adicional'],
                "2" => $stockHtml,
                "3" => $_SESSION['simbolo_str'].formatMoney($precio),
                "4" => $reg['lab_nombre'], 
                "5" => $reg['tipo_nombre'],
                "6" => $reg['present_nombre'],
                "7" => '<button id="btnAgregarP" type="button" class="btn btn-info btn-sm" title="Aregar" name="'.$reg['prod_id'].'" onclick="addDetail('.$reg['prod_id'].',\''.$reg['prod_nombre'].'\',\''.$reg['prod_concentracion'].'\',\''.$reg['prod_adicional'].'\','.$precio.','.$cantidad.')"><span class="fa fa-plus fa-xs"></span></button>'
            );
        }
    ...
    public function barcode_prod_controller(){ 
        ...
        while($row = $resp->fetch()) {
            ...
            $result[0] = array(
                ...
                'precio' => isset($row['prod_precioV']) ? $row['prod_precioV'] : 0,
                'cantidad' => isset($stock['lote_stock']) ? $stock['lote_stock'] : 0
            );
        }
